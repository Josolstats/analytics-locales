...
app = Flask(__name__)
app.secret_key = "maxxage_analytics_2026_secret"

USUARIOS = {
    "jose": hashlib.sha256("Jsnou6".encode()).hexdigest()
}
...
<style>
:root{--bg:#0a0a0f;--sur:#111118;--bor:#1e1e2e;--txt:#e8e8f0;--mu:#5a5a7a;--ok:#3ecfb2;--err:#ff4d6d}
...
.logo{font-family:'Syne',sans-serif;font-weight:800;font-size:clamp(2rem,6vw,3.5rem);letter-spacing:-.03em;margin-bottom:.5rem;text-align:center}
.logo span{color:var(--ok)}
.sub{font-size:.65rem;letter-spacing:.2em;text-transform:uppercase;color:var(--mu);margin-bottom:3rem;text-align:center}
.card{background:var(--sur);border:1px solid var(--bor);border-radius:12px;padding:2rem;width:100%;max-width:360px}
.field{margin-bottom:1.25rem}
label{font-size:.65rem;letter-spacing:.12em;text-transform:uppercase;color:var(--mu);display:block;margin-bottom:.5rem}
input{width:100%;background:var(--bg);border:1px solid var(--bor);border-radius:6px;padding:.75rem 1rem;color:var(--txt);font-family:'DM Mono',monospace;font-size:.85rem;outline:none;transition:border .2s}
input:focus{border-color:var(--ok)}
button{width:100%;background:transparent;border:1px solid var(--ok);color:var(--ok);font-family:'DM Mono',monospace;font-size:.75rem;letter-spacing:.1em;text-transform:uppercase;padding:.85rem;border-radius:6px;cursor:pointer;transition:all .2s;margin-top:.5rem}
button:hover{background:var(--ok);color:var(--bg)}
.error{color:var(--err);font-size:.7rem;margin-top:1rem;text-align:center}
</style>
</head>
<body>
<div class="logo">Maxxage <span>Analytics</span></div>
<div class="sub">Panel de informes</div>
<div class="card">
...
<style>
:root{--bg:#0a0a0f;--sur:#111118;--bor:#1e1e2e;--bor2:#2a2a3e;--txt:#e8e8f0;--mu:#5a5a7a;--ok:#3ecfb2;--err:#ff4d6d;--blu:#5b8dee}
*{box-sizing:border-box;margin:0;padding:0}
...
.logo{font-family:'Syne',sans-serif;font-weight:800;font-size:clamp(1.5rem,4vw,2.5rem);letter-spacing:-.03em}
.logo span{color:var(--ok)}
.logout{font-size:.65rem;letter-spacing:.1em;text-transform:uppercase;color:var(--mu);text-decoration:none;border:1px solid var(--bor2);padding:.4rem .8rem;border-radius:4px;transition:all .2s}
...
function cargarMes(){
  ...
  fetch(`/api/mes/${localActual}/${mes}`)
    ...
    }).catch(e=>{clearInterval(iv);document.getElementById('loading-mes').style.display='none';document.getElementById('contenido-mes').style.opacity='1';alert('Error: '+e);});
}

function renderMes(data){
  const mes=parseInt(document.getElementById('sel-mes').value);
  const {y2025:t25,y2026:t26,hasta_dia}=data;
  document.getElementById('badge-hasta').textContent=`${MESES[mes-1]} hasta día ${hasta_dia}`;
  ...
  if(!window.datosMeses) window.datosMeses={};
  window.datosMeses[mes]={y25:t25.ing,y26:t26.ing};
  const labelsX=[],arr25=[],arr26=[];
  for(let m=1;m<=4;m++){if(window.datosMeses[m]){labelsX.push(MESES[m-1]);arr25.push(window.datosMeses[m].y25);arr26.push(window.datosMeses[m].y26);}}
  if(chartMes) chartMes.destroy();
  chartMes=new Chart(document.getElementById('chart-mes'),{type:'bar',
    data:{labels:labelsX,datasets:[
      {label:'2025',data:arr25,backgroundColor:labelsX.map((_,i)=>labelsX[i]===MESES[mes-1]?'rgba(91,141,238,0.9)':'rgba(91,141,238,0.4)'),borderRadius:3},
      {label:'2026',data:arr26,backgroundColor:labelsX.map((_,i)=>labelsX[i]===MESES[mes-1]?'rgba(62,207,178,0.9)':'rgba(62,207,178,0.4)'),borderRadius:3},
    ]},
...
function renderDetalle(data){
  ...
  chartDiario=new Chart(document.getElementById('chart-diario'),{type:'bar',
    data:{labels:d26.map(r=>r.fecha),datasets:[
  ...
  const rows=d26.map((r,i)=>{
    ...
    return `<tr><td>${r.fecha}</td><td>${fmt(r25.ing)}</td><td>${fmt(r.ing)}</td>
      <td class="${cls}">${df>=0?'+':''}${fmt(df)}</td>
      <td class="${cls}">${df>=0?'▲':'▼'}${Math.abs(pp).toFixed(1)}%</td>
      <td>${fmt(r25.bar)}</td><td>${fmt(r.bar)}</td>
      <td>${fmt(r25.ent)}</td><td>${fmt(r.ent)}</td></tr>`;
  });
  ...
  document.getElementById('tfoot-diario').innerHTML=`<tr>
    <td>Total</td><td>${fmt(tot25.ing)}</td><td>${fmt(tot26.ing)}</td>
    ...
    <td>${fmt(tot25.ent)}</td><td>${fmt(tot26.ent)}</td></tr>`;
}
...
function cargarAcum(){
  ...
  let prog=0;
  const iv=setInterval(()=>{prog=Math.min(prog+Math.random()*10,90);document.getElementById('progress-acum').style.width=prog+'%';},600);
...
function renderAcum(data){
  ...
  const labels=['Enero','Febrero','Marzo','Abril'].slice(0,meses2026.length);
  if(chartAcum) chartAcum.destroy();
  chartAcum=new Chart(document.getElementById('chart-acum'),{type:'line',
...
def login():
    ...
    if request.method == "POST":
        ...
        if usuario in USUARIOS and USUARIOS[usuario] == hashed:
...
def api_mes(local_key, mes):
    ...
    try:
        return jsonify(get_mes(local_key, mes))
...
def api_mes_dias(local_key, mes):
    if local_key not in LOCALES:
        return jsonify({"error": "Local no encontrado"}), 404
    try:
        return jsonify(get_mes_con_dias(local_key, mes))
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_acumulado(local_key):
    if local_key not in LOCALES:
        return jsonify({"error": "Local no encontrado"}), 404
    try:
        return jsonify(get_acumulado(local_key))
    except Exception as e:
        return jsonify({"error": str(e)}), 500

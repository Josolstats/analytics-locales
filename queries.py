...
LOCALES = {
    "sala_maxx":    {"nombre": "Sala Maxx",          "db_rec": "bs2026",       "db_bar": "barrabs2026",  "num_barras": 5, "entradas": True,  "cocktails": True},
    "sala_kixx":    {"nombre": "Sala Kixx",           "db_rec": "kixx2026",     "db_bar": "barrakixx2026","num_barras": 2, "entradas": True,  "cocktails": True},
    "luna_azul":    {"nombre": "Luna Azul",           "db_rec": "lunaxilxes2026","db_bar": "barraluna2026","num_barras": 1, "entradas": True,  "cocktails": True},
    "mb_ibiza":     {"nombre": "MB Ibiza",            "db_rec": "mbi2026",      "db_bar": None,           "num_barras": 0, "entradas": False, "cocktails": False},
    "maxxage_mad":  {"nombre": "Maxxage Madrid",      "db_rec": "maxxmad2026",  "db_bar": None,           "num_barras": 0, "entradas": False, "cocktails": False},
    "maxxage_vlc":  {"nombre": "Maxxage Valencia",    "db_rec": "maxxvlc2026",  "db_bar": None,           "num_barras": 0, "entradas": False, "cocktails": False},
    "mb_marbella":  {"nombre": "MB Marbella",         "db_rec": "mbm2026",      "db_bar": None,           "num_barras": 0, "entradas": False, "cocktails": False},
}
...
def calcular_dia(conn, db_rec, db_bar, local_key, lote, fecha, anyo):
    cur = conn.cursor()
    d, m, y = fecha.day, fecha.month, fecha.year
    cfg = LOCALES[local_key]

    # Pensiones = cobros del año - alquileres
    ...
    cur.execute(f"""
        ...
        FROM {db_rec}.tickets t, {db_rec}.lineasticket l
        WHERE t.codlot={lote} AND t.codtic=l.codtic AND l.codart < 100000
    """)
    ...
    # Barras
    bar = 0
    if db_bar and cfg["num_barras"] > 0:
        for b in range(1, cfg["num_barras"] + 1):
            try:
                cur.execute(f"""
                    SELECT SUM(l.cantidad*l.precio)
                    FROM {db_bar}.lotes{b} lo, {db_bar}.lineasticket{b} l, {db_bar}.tickets{b} t
                    WHERE lo.codlot=t.codlot AND l.codtic=t.codtic
                    AND (t.estado=1 OR t.estado=2)
                    AND DAY(lo.fecha)={d} AND MONTH(lo.fecha)={m} AND YEAR(lo.fecha)={y}
                """)
    ...
    # Entradas
    ent = 0
    if db_bar and cfg["entradas"]:
        try:
            cur.execute(f"""
                SELECT SUM(v.precio)
                FROM {db_bar}.lotes_entradas lo, {db_bar}.entradas e, {db_bar}.var_entradas v
                WHERE lo.codlotent=e.codlot AND v.tipo=e.tipo
                AND DAY(lo.fecha)={d} AND MONTH(lo.fecha)={m} AND YEAR(lo.fecha)={y}
            """)
    ...
    # Cocktails
    coc = 0
    if db_bar and cfg["cocktails"]:
        for b in range(1, cfg["num_barras"] + 1):
            try:
                cur.execute(f"""
                    SELECT SUM(c.tickets_sorteo*5)
                    FROM {db_bar}.cocktails c, {db_bar}.lotes{b} lo
                    WHERE lo.codlot=c.lote AND c.barra={b}
                    AND DAY(lo.fecha)={d} AND MONTH(lo.fecha)={m} AND YEAR(lo.fecha)={y}
                """)
    ...
    try:
        cur.execute(f"""
            SELECT SUM(g.importe) FROM {db_rec}.gastos g
            WHERE g.codlot={lote} AND g.estado<>2 AND g.codcon NOT IN (23, 30)
        """)
        gas = float(cur.fetchone()[0] or 0)
    except Exception:
        gas = 0
    ...
    return {
        "pen": round(pen), "alq": round(alq), "bar": round(bar),
        "ent": round(ent), "coc": round(coc), "ing": round(ing),
        "gas": round(gas), "dis": round(dis)
    }

def get_totales_rango(local_key, anyo, fecha_ini, fecha_fin, incluir_dias=False):
    cfg = LOCALES[local_key] if anyo == 2026 else {**LOCALES[local_key], **LOCALES_2025.get(local_key, {})}
    if anyo == 2025:
        db_rec = LOCALES_2025[local_key]["db_rec"]
        db_bar = LOCALES_2025[local_key]["db_bar"]
    else:
        db_rec = LOCALES[local_key]["db_rec"]
        db_bar = LOCALES[local_key]["db_bar"]
    ...
    try:
        cur.execute(f"""
            SELECT codlot, fecha FROM {db_rec}.lotes
        ...
        for lote, fecha in lotes:
            d = calcular_dia(conn, db_rec, db_bar, local_key, lote, fecha, anyo)
